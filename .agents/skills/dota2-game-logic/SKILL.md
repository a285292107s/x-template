...
4. **配置与代码分离**：所有可能与前端共享或需要集中管理的配置、数值、常量（包括但不限于技能属性、物品数据、单位属性、游戏规则参数、界面配置等），都必须放在 `game/scripts/npc/` 下的 KV 文件中。后端代码通过编译后的 JSON 读取，严禁在代码中硬编码任何可能变化的配置值。修改配置后需运行 `npx gulp kv_2_js` 刷新 JSON
5. **编译验证**：代码修改完成后，必须运行编译验证，确保无错误
...
### 步骤 2b：编译 KV 到 JSON（如修改了 KV 文件）

如果修改了 `game/scripts/npc/` 下的 KV 文件，必须运行以下命令将 KV 编译为 JSON，保证前后端数据一致：
...
**前后端数据分离原则**：
- 所有游戏数值配置写在 KV 文件中，前后端都从编译后的 JSON 读取同一份数据
- 禁止在后端代码中硬编码数值常量，所有值必须从 KV → JSON 获取
