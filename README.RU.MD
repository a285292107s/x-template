...
6. **Jest-совместимая тестовая система** — пишите и запускайте тесты внутри игры через чат, см. [руководство](https://github.com/XavierCHN/x-template/blob/master/game/scripts/src/utils/testing/readme.md)
7. Анализ производительности через флейм-графы - см. [руководство](https://github.com/XavierCHN/x-template/blob/master/game/scripts/src/utils/performance/flame_graph_profiler.md)
...
```bash
# 1. Убедитесь, что ключи публикации настроены (см. Конфигурация)

# 2. Выполните публикацию
yarn prod

# Процесс публикации автоматически выполняет:
# - gulp predev (Excel → KV → JSON, CSV → локализация, предварительное кэширование изображений)
# - Сборку Webpack для production (минимизация)
# - Компиляцию TSTL для production
# - Шифрование кода (согласно конфигурации encrypt_files)
# - Копирование в директорию публикации
```

### Директория публикации

Опубликованные файлы выводятся в `dota 2 beta/game/dota_addons/{addon_name}_publish/`, готовые для загрузки в Мастерскую Dota 2.

Используйте `yarn launch {addon_name}_publish` для прямого запуска инструментов Мастерской для загрузки.
...
### Навыки разработки с ИИ

Проект включает набор **навыков разработки с ИИ (Skills)** в `.agents/skills/`, предназначенных для руководства ИИ-ассистентами (например, OpenCode) при понимании и генерации кода для конкретных областей:

| Навык | Описание |
|-------|----------|
| `dota2-ability` | Создание пользовательских способностей Dota 2 |
| `dota2-game-logic` | Разработка серверной логики игры |
| `dota2-item` | Создание пользовательских предметов |
| `dota2-localization` | Генерация текстов локализации |
| `dota2-ui` | Разработка Panorama UI |
| `dota2-unit` | Создание пользовательских юнитов |

Эти навыки являются **постоянно оптимизируемыми тестовыми модулями**, определяющими стандарты, соглашения и лучшие практики, которым должен следовать ИИ при генерации кода. Приветствуются улучшения от сообщества через Pull Requests.
